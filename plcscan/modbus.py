...
class ModbusError(Exception):
    ...
    def __init__(self,  code):
        self.code = code
        self.message = ModbusError._errors[code] if ModbusError._errors.get(code) else 'Unknown Error'
    def __str__(self):
...
class ModbusPacket:
    ...
    def pack(self):
        packet_data = self.data if isinstance(self.data, bytes) else self.data.encode()
        return struct.pack('!HHHBB',
            self.transactionId,   # transaction id
            0,                    # protocol identifier (reserved 0)
            len(packet_data) + 2,  # remaining length
            self.unitId,          # unit id
...
def ScanUnit(ip, port, uid, timeout, function=None, data=''):
    ...
    if function:
        try:
            ...
            unitInfo.append("Response: %s\t(%s)" % (StripUnprintable(response), binascii.hexlify(response).decode('utf-8')))
...
def Scan(ip, port, options):
    res = False
    try:
        data = bytes(options.modbus_data, 'utf-8').decode('unicode_escape') if options.modbus_data else b''
